...
async function containsBlacklistedUrl(text) {
  ...
  for (const url of urls) {
    ...
    if (result.startsWith('200 Ok')) {
      ...
      if (flags === 'true true') return { blacklisted: true, url };
...
exports.updateMail = async (req, res) => {
  ...
  const check = await containsBlacklistedUrl(text);
  if (check.error) return res.status(500).json({ error: `C++ error: ${check.url}` });
  if (check.blacklisted) return res.status(400).json({ error: `Blacklisted URL: ${check.url}` });
